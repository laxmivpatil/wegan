...
public interface OrderItemRepository extends JpaRepository<OrderItem, Long> {
	 ...
	 @Query("SELECT oi FROM OrderItem oi WHERE oi.product.user.id = :sellerId")
	   ...
	   @Query("SELECT oi FROM OrderItem oi WHERE oi.product.user.id = :sellerId AND oi.orderItemStatus = :orderStatus1 OR oi.orderItemStatus = :orderStatus2")
	    List<OrderItem> findAllBySellerIdAndOrderStatus(Long sellerId, String orderStatus1,String orderStatus2);
